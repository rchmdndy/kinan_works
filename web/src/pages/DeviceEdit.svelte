<script lang="ts">
  ...
  $effect(() => {
    if (device.id !== activeDeviceId) {
      ...
      label = device.label;
      parameters = Object.values(device.parameters).map((parameter) => ({
        ...parameter,
      }));
      message = '';
      notice = '';
  ...
  async function saveDevice() {
    ...
    try {
      ...
      const updated = parameters.map((parameter, index) => ({
        id: parameter.id,
      ...
      parameters = Object.values(result.device.parameters).map((parameter) => ({
        ...parameter,
      }));
...
<section id="device-editor" class="panel editor-panel">
