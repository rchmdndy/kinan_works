<script lang="ts">
  let { busy = false, message = '', onsubmit }: { busy?: boolean; message?: string; onsubmit: (username: string, password: string) => void } = $props();
  let username = $state('');
  let password = $state('');
...
<main class="auth-shell">
  <section class="auth-card" aria-labelledby="login-title">
    ...
    <form onsubmit={submit}>
      <label>Username<input type="text" bind:value={username} required autocomplete="username" disabled={busy} /></label>
      <label>Password<input type="password" bind:value={password} required minlength="6" autocomplete="current-password" disabled={busy} /></label>
