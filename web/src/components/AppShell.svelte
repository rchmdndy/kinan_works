<script lang="ts">
  ...
  const devicesActive = (page: Route['page']) =>
    ['devices', 'detail', 'edit', 'realtime', 'new'].includes(page);
</script>
...
<div class="app-layout">
  <aside class="sidebar">
    ...
    <nav aria-label="Navigasi utama">
      <a href="#devices" class:active={devicesActive(route.page)}>
        <svg viewBox="0 0 24 24" aria-hidden="true"
          ><rect x="4" y="3" width="16" height="18" rx="1"></rect><path
      ...
      <a href="#exports" class:active={route.page === 'exports'}>
        <svg viewBox="0 0 24 24" aria-hidden="true"
          ><path d="M12 3v12m0 0 4-4m-4 4-4-4"></path><path d="M5 17v3h14v-3"
          ></path></svg
        >
        Exports
      </a>
    ...
    <div class="sidebar-account">
      <span class="avatar"
        >{(user.username || 'U').slice(0, 1).toUpperCase()}</span
      ><span
        ><strong>{user.username}</strong><button
          class="text-button"
          onclick={onlogout}>Keluar</button
        ></span
      >
    </div>
  </aside>
