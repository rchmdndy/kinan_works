...
describe('hash routes', () => {
  test('parses each application route', () => {
    ...
    expect(parseHashRoute('#/devices/a%2Fb')).toEqual({
      page: 'detail',
      id: 'a/b',
    });
    expect(parseHashRoute('#/devices/a/edit')).toEqual({
      page: 'edit',
      id: 'a',
    });
    expect(parseHashRoute('#/devices/a/realtime')).toEqual({
      page: 'realtime',
      id: 'a',
    });
    expect(parseHashRoute('#/devices/a/credential')).toEqual({
      page: 'credential',
      id: 'a',
    });
