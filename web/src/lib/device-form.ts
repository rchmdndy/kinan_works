...
export function validateDeviceDraft(
  ...
  else if (cleanLabel.length > 100)
    errors.label = 'Nama maksimal 100 karakter.';
  if (!parameters.length)
    errors.parameters = 'Tambahkan minimal satu parameter.';
  else if (parameters.length > 100)
    errors.parameters = 'Maksimal 100 parameter.';

  parameters.forEach((parameter, index) => {
    const cleanParameterLabel = parameter.label.trim();
    if (!cleanParameterLabel)
...
export function hasDeviceDraftErrors(errors: DeviceDraftErrors): boolean {
  return Boolean(
    errors.label ||
      errors.parameters ||
      errors.parameter.some((parameter) => Object.keys(parameter).length),
  );
}

export function normalizeDeviceDraft(
  label: string,
  parameters: ParameterDraft[],
) {
  return {
