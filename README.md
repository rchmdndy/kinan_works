...
- Exports run in a dedicated internal microservice (`exporter`). The browser only submits an export job through the API, polls its status, and downloads the finished file; XLSX generation never happens in the browser. The exporter opens the shared SQLite telemetry database read-only, walks history with keyset pagination (`timestamp`, `write_id`) in bounded pages, and enforces per-user active-job limits, a maximum row guard, and file retention. `TELEMETRY_MAX_SAMPLES` bounds a single history query, not total export rows: exports are not limited to 500 samples. Export routes on the main API (`GET/POST /api/exports`, `GET /api/exports/:id`, `GET /api/exports/:id/file`) enforce sessions, CSRF on creation, device ownership, and a per-user rate limit before forwarding to `EXPORTER_URL`; the exporter itself is never exposed publicly and holds no credentials.
...
Dashboard parameters are authoritative and have exactly three types: `nilai` (numeric sensor), `control-state` (boolean switch), and `control-setpoint` (numeric target with finite `min < max`). There is no mode control or separate device-advertised capability list. Firmware uses stable server-generated parameter IDs. The dashboard can overwrite a saved parameter's type, label, unit, precision, and setpoint bounds without changing its ID or creating metadata versions. Switches have no unit or decimal inputs. New parameters default to `nilai`; existing parameters migrate to `nilai` without changing IDs or history. To add a parameter through PATCH, omit its ID; unknown client-supplied IDs are rejected. Type changes do not convert or delete existing telemetry, state, or command records: dashboards and exports use the latest parameter definition, and new payloads must match that definition. Removed IDs are not reused. The dedicated Growth Chamber profile still fails closed when its required IDs or types no longer match.
