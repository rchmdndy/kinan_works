...
try {
  if (command === 'create-user') {
    ...
    const displayName = args[1]?.trim() || username;
    if (!username || !/^[a-zA-Z0-9_.-]{3,64}$/.test(username))
      throw new Error('Usage: admin create-user <username> [display-name]');
    ...
    const legacyFlag = args.indexOf('--legacy-uid');
    const usernameFlag = args.indexOf('--username');
    const legacyUid = legacyFlag >= 0 ? args[legacyFlag + 1] : undefined;
    const username = usernameFlag >= 0 ? args[usernameFlag + 1] : undefined;
    if (!legacyUid || !username)
      throw new Error(
        'Usage: admin link-owner --legacy-uid <uid> --username <username>',
      );
    const user = repository.getUserByUsername(username);
    if (!user) throw new Error('Local username not found');
    console.log(
      `Linked ${repository.linkLegacyOwner(legacyUid, user.id)} device(s) from explicit legacy UID to ${username}`,
    );
