...
const envSchema = z.object({
  ...
  SESSION_TTL_MS: z.coerce.number().int().min(60_000).max(31_536_000_000).default(86_400_000),
  ...
  TELEMETRY_MAX_BYTES: z.coerce.number().int().positive().max(1_048_576).default(131_072),
  TELEMETRY_MAX_CLOCK_SKEW_MS: z.coerce.number().int().nonnegative().default(300_000),
  RETENTION_DAYS: z.coerce.number().int().positive().default(30)
});

export type Config = z.infer<typeof envSchema> & { encryptionKey: Uint8Array };

const requiredConfig = ['ENCRYPTION_KEY_BASE64', 'MQTT_INGEST_PASSWORD'] as const;
