...
export const usernameSchema = z.string().trim().toLowerCase().regex(/^[a-z0-9][a-z0-9_.-]{2,63}$/);
...
const sensorValueSchema = z.discriminatedUnion('status', [
  ...
  z.strictObject({ status: z.literal('error'), error: z.string().trim().min(1).max(200) })
]);

export const telemetrySchema = z.strictObject({
  credentialVersion: z.number().int().positive(),
  writeId: z.string().regex(/^[a-zA-Z0-9_-]{8,128}$/),
  values: z.record(z.string().regex(/^[a-zA-Z0-9_-]{1,64}$/), sensorValueSchema),
